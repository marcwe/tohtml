...
fin = open('/usr/share/nginx/www/phpdata_a.txt', 'r')
foutlog = open('phpfileslog_a.txt' , 'w')
fout = open('phpfiles_a.txt' , 'w')
foutneedswork = open('needswork_a.txt' , 'w')
...
#		fout.write ("\n")
	if len(row) == 3 :
		if ("http" in row[0] and "node-" in row[1]):
			fout.write (str(row[0]))
			fout.write ("??")
			fout.write (str(row[1]))
			fout.write ("\n")
